...
% 主程式開始
% 設定參數值
C_values = [1.00, 1.33, 1.5, 1.67, 2.00];  % C 值序列
n_values = 10:5:405;                        % 樣本數序列（從10到405，間隔5）
alpha_values = [0.01, 0.025, 0.05];         % 顯著性水平序列

% 創建儲存結果的陣列
num_n = length(n_values);          % 樣本數數量
num_alpha = length(alpha_values);  % 顯著性水平數量
num_C = length(C_values);          % C值數量
...
for c_idx = 1:num_C
    ...
    for i = 1:num_n
        ...
        % 根據樣本數決定 Cp 值
        if n < 100
            Cp = C + 0.33;  % 樣本數小於100時的調整
        else
            Cp = C + 0.12;  % 樣本數大於等於100時的調整
        end
    ...
    % 列印結果數據
    for i = 1:num_n
        fprintf('%6d', n_values(i));  % 列印樣本數
        for j = 1:num_alpha
            fprintf('%11.3f', results(i, j));  % 列印計算結果，精確到小數點後3位
        end
        fprintf('\n');  % 換行
    end
    fprintf('---------------------------------------------------------------\n\n');
