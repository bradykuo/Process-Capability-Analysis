...
calculate_critical_value <- function(C = 1.00, n = 38, alpha = 0.05, r_initial = 1.260) {
  ...
  # 定義 G 函數 (對應 Maple 程式中的卡方分配累積機率函數)
  G <- function(c0, y) {
    pchisq((n - 1) * (3 * Cp * n^0.5 - y)^2 / (9 * n * c0^2), df = n-1)
  }
  
  # 定義 f 函數 (對應 Maple 程式中的摺疊常態分配密度函數)
  f <- function(y) {
...
# 印出結果
cat("參數設定：\n")
cat(sprintf("製程能力要求 C = %.2f\n", result$parameters$C))
cat(sprintf("樣本數 n = %d\n", result$parameters$n))
cat(sprintf("顯著水準 α = %.2f\n", result$parameters$alpha))
cat(sprintf("製程特性參數 Cp = %.2f\n", result$parameters$Cp))
