...
create_cpk_plot <- function(Cpk, alpha = 0.05) {
  ...
  p <- ggplot(plot_data, aes(x = Cp, y = c0, color = factor(n))) +
    ...
    scale_color_discrete(name = "樣本數 (n)") +  # 設定圖例名稱
    labs(title = sprintf("Cpk = %.2f", Cpk),   # 設定標題和軸標籤
         x = "Cp",
         y = "c0") +
    ...
    coord_cartesian(ylim = c(y_min, y_max), expand = FALSE)  # 設定y軸範圍
...
for(i in seq_along(plots)) {
  ggsave(
    filename = sprintf("cpkplot%.2f.png", Cpk_values[i]),
    plot = plots[[i]],
    width = 8,
    height = 6,
    dpi = 300
