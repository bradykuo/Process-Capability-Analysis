...
for mu_idx = 1:length(mu_list)
    ...
    for sigma_idx = 1:length(sigma_list)
        ...
        % Calculate scaling parameters for chi-squared distribution
        % Chi-squared(k) has mean=k and variance=2k
        % We need to scale it to match our desired mean and variance
        scale = sigma/sqrt(2*df);
        shift = mu - df*scale;
        ...
        for n_idx = 1:length(n_list)
            ...
            for sim = 1:N
                % Generate original sample from chi-squared distribution
                sample_fixed = chi2rnd(df, [1,n])*scale + shift;
