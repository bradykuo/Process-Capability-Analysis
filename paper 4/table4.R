...
for(mu_idx in 1:length(mu_list)) {
  ...
  for(sigma_idx in 1:length(sigma_list)) {
    ...
    for(n_idx in 1:length(n_list)) {
      ...
      for(sim in 1:N) {
        # 從卡方分配生成原始樣本
        sample_fixed <- rchisq(n, df = df) * scale + shift
        ...
        # 計算 Biased Corrected Percentile Bootstrap 信賴區間下限
        # 首先計算原始估計值落在 Bootstrap 分配中的位置
